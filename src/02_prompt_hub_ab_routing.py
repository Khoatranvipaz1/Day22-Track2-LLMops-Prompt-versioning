...
def push_prompts_to_hub(client: Client):
    ...
    try:
        url = client.push_prompt(PROMPT_V1_NAME, object=PROMPT_V1, description="V1 - concise grounded RAG")
    ...
    try:
        url = client.push_prompt(PROMPT_V2_NAME, object=PROMPT_V2, description="V2 - structured expert RAG")
...
def main():
    ...
    skip_hub = config.os.getenv("SKIP_LANGSMITH_HUB", "false").lower() in ("1", "true", "yes")
    if skip_hub:
        print("ℹ️  Bỏ qua Prompt Hub, dùng prompt local cho chế độ offline.")
