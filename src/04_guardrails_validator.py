...
class PIIDetector(Validator):
    ...
    PII_PATTERNS = {
        ...
        "PHONE":       r"(?<!\d)(?:\+?1[-.\s]?)?(?:\(\d{3}\)|\d{3})[-.\s]?\d{3}[-.\s]?\d{4}\b",
...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        try:
            ...
            return FailResult(
                error_message="Invalid JSON repaired automatically",
                fix_value=json.dumps(parsed, indent=2),
            )
        except json.JSONDecodeError as e:
            return FailResult(
                error_message=f"JSON không hợp lệ sau khi sửa: {e}",
                fix_value='{"error": "invalid_json", "message": "Could not repair output"}',
            )
...
# ── 5. Main ────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Run Guardrails validator demos.")
    parser.add_argument(
        "--demo",
